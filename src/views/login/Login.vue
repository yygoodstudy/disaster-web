<template>
  <div class="login">
    ...
    <!-- 登录面板 -->
    <div class="login-box">
      <div class="login-box-title">灾情上报后台管理系统</div>
        ...
        .then((res) => {
          ...
          if (res.data.code === 20000) {
            this.$store
              .dispatch("user/login", {
                username: this.loginForm.username,
                code: res.data.code, // code：服务器返回的状态码用于判断用户是否登录成功，如果登录成功将username存入到token中
              }) // 这里的user/login是调用store/user.js中login方法
              .then(() => {
                this.loading = true;
                // 登陆成功后重定向
                this.$router.push({
